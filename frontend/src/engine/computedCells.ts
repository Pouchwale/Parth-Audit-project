...
// §68). Three families of form print a cell that is arithmetic on the cells
// beside it: the calibration records' deviations (engine/calibration.ts), the
// purchase registers' weighted ratings (engine/purchaseRatings.ts) and the
// breakdown register's total minutes (engine/maintenanceCalc.ts). Each used to
...
export function withComputedCells<T>(documentId: string | undefined, data: T): T {
  return withMaintenanceCalc(documentId, withCalibration(documentId, withPurchaseRatings(documentId, data)));
}
