...
const ACTIVITY_WORDS: Record<HistoryAction, string> = {
  ...
  reopened: "Record reopened for correction",
...
export function recordLabel(record: Pick<RecordInstance, "documentId" | "dueDate">): string {
  ...
  const number = doc && !doc.formatNo.startsWith("TO BE") ? `${doc.formatNo} ` : "";
  return `${number}${doc?.name ?? record.documentId} — ${record.dueDate}`;
}

// One line per NEW history entry (REQUIREMENTS §62). Not for demo records,
// which are generated by the hundred; not for what the system does by itself
// at start-up (the assistant's preparation, the clean-ups), which nobody did;
// and not for an autosave folded into the entry before it, which is the same
// sitting's same edit.
function logEntry(record: RecordInstance, entry: HistoryEntry): void {
  if (record.isDemo || entry.action === "prepared" || entry.by === "System" || entry.by === "Assistant") return;
  const changed = entry.changes?.length ? `${entry.changes.length + (entry.moreChanges ?? 0)} field(s): ${entry.changes.slice(0, 4).map((c) => c.label).join(", ")}` : "";
  logActivity(ACTIVITY_WORDS[entry.action] ?? entry.action, recordLabel(record), [entry.note, changed].filter(Boolean).join(" · "), record.documentId);
}
...
export function appendHistory<T>(record: RecordInstance<T>, entry: HistoryEntry): RecordInstance<T> {
  ...
  if (foldable && entry.changes) {
    ...
    // Folded into the entry before it: the same edit, already in the log.
  ...
  history.push(entry);
  logEntry(record as RecordInstance, entry);
  return { ...record, history };
}
