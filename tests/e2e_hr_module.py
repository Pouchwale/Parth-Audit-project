...
everything in HR Module". So this suite checks that:
  ...
  * the Document Library holds ninety-one documents, and the Human Resources module
  ...
  * the library holds ninety-one documents (REQUIREMENTS s51 added Quality
...
with sync_playwright() as p:
    ...
    # ==================================================================
    open_library(page)
    check("The Document Library lists ninety-one documents", library_rows(page).count() == 91, library_rows(page).count())
    group = hr_group(page)
    ...
    check("Every document in the library has Open Document", len(all_ids) == 91, len(all_ids))
    landed = {}
